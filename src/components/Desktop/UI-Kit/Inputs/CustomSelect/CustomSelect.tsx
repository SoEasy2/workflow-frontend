...
const Component: React.FC<ISelect> = ({
  ...
  const handleClick = (option: IOption) => {
    setValue(option);
    modelValue && onChange && onChange && onChange(name, { ...modelValue[name], value: option.value.toString() });
    setOpen(false);
  };
  ...
  useEffect(() => {
    ...
    const searchOption = modelValue && modelValue[name] ? options.find(item => item.value === Number(modelValue[name].value)) : null;
